...
extern template class TLexer<Lexer::type>;

void tokenize_and_parse(std::istream& in, std::ostream& json, std::ostream& code, std::ostream& tables) {
...
int main(int argc, char* argv[]) {
    ...
    for(int i = 1; i < argc; ++i) {
        ...
        std::ofstream json(argv[i] + std::string(".json"));
        std::ofstream code(argv[i] + std::string(".code"));
        std::ofstream tables(argv[i] + std::string(".tables"));
        in.unsetf(std::ios::skipws);
        tokenize_and_parse(in, json, code, tables);
    }
}
